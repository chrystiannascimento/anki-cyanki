<script lang="ts">
    ...
    async function loadSessionData() {
        ...
        let results = await db.flashcards.where('id').anyOf(ids).toArray();
    ...
    function flipCard() { isFlipped = true; }
    function recordAnswer(correct: boolean) {
        if (correct) correctCount++; else wrongCount++;
        nextCard();
    }
    function nextCard() {
        if (currentCardIndex < flashcards.length - 1) { currentCardIndex++; isFlipped = false; }
    ...
    <main class="w-full max-w-2xl flex flex-col items-center justify-center mt-16 z-0">
        ...
        {:else if isFinished}
            <div class="bg-neutral-800/60 backdrop-blur-md p-10 rounded-3xl w-full text-center border border-neutral-700 shadow-2xl relative overflow-hidden">
                ...
                <p class="text-neutral-400 mb-8 max-w-sm mx-auto">Você esgotou todas as questões deste caderno. Retorne para ler suas notas ou revisar.</p>
                <div class="flex flex-col md:flex-row gap-4 justify-center">
                    <button on:click={finishSession} class="px-8 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl shadow-lg shadow-indigo-500/25 transition-all text-sm uppercase tracking-wider hover:-translate-y-1">Encerrar Sessão</button>
                    <button on:click={async () => { isFinished = false; isLoading = true; correctCount = 0; wrongCount = 0; currentCardIndex = 0; await loadSessionData(); }} class="px-8 py-3.5 bg-neutral-700 hover:bg-neutral-600 text-white font-bold rounded-xl shadow border border-neutral-600 transition-all text-sm uppercase tracking-wider hover:-translate-y-1">Refazer Conjunto</button>
                </div>
            </div>
            ...
            <div class="mt-8 md:mt-12 w-full max-w-md">
                ...
                {:else}
                    <div class="grid grid-cols-2 gap-4 animate-fade-in-up">
                        <button on:click={() => recordAnswer(false)} class="py-4 bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 border border-rose-500/30 font-extrabold rounded-2xl transition-all active:scale-95 text-lg shadow-lg hover:shadow-rose-500/20">Errei</button>
                        <button on:click={() => recordAnswer(true)} class="py-4 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 border border-emerald-500/30 font-extrabold rounded-2xl transition-all active:scale-95 text-lg shadow-lg hover:shadow-emerald-500/20">Acertei</button>
                    </div>
...
<svelte:window on:keydown={(e) => {
    ...
    else if (e.code === 'Digit1' && isFlipped) recordAnswer(false);
    else if (e.code === 'Digit2' && isFlipped) recordAnswer(true);
}}/>
