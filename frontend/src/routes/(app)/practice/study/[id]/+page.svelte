<script lang="ts">
    ...
    async function loadDueFilteredCards() {
        ...
        // 3. FSRS Integration - Cross-reference pool with Card States
        const cardStates = await getAllCardStates();
        const now = new Date();

        // 4. Extract Due Cards from the Caderno
        const overdueCards = pool.filter(c => {
            const state = cardStates.get(c.id);
            return state && state.due <= now;
        });

        // 5. Sort by age (most overdue first as standard practice)
    ...
    <main class="w-full max-w-2xl flex flex-col items-center justify-center mt-16 z-0">
        ...
        {:else if !currentCard && dueCards.length === 0}
            <div class="bg-neutral-800/60 backdrop-blur-md p-10 rounded-3xl w-full text-center border border-neutral-700 shadow-2xl relative overflow-hidden">
                ...
                <p class="text-neutral-400 mb-8 max-w-sm mx-auto">Você não tem questões programadas ou atrasadas para o algoritmo FSRS neste momento para essa configuração de filtro específica.</p>
        ...
        {:else if currentIndex >= dueCards.length}
            <div class="bg-neutral-800/60 backdrop-blur-md p-10 rounded-3xl w-full text-center border border-neutral-700 shadow-2xl relative overflow-hidden">
                ...
                <p class="text-neutral-400 mb-8 max-w-sm mx-auto">Sua meta foi batida. Todas as revisões programadas FSRS para as questões deste Caderno foram processadas organicamente.</p>
